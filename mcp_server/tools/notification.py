...
from aiyxdata_tradar.core.loader import _load_webhook_config, _load_notification_config
from aiyxdata_tradar.notification.batch import (
    truncate_to_bytes,
    get_batch_header,
    get_max_batch_header_size,
    add_batch_headers,
)
from aiyxdata_tradar.notification.formatters import strip_markdown
from aiyxdata_tradar.notification.senders import SMTP_CONFIGS
...
# add_batch_headers 复用自 aiyxdata_tradar.notification.batch


def _split_text_into_batches(text: str, max_bytes: int) -> List[str]:
    """将文本按字节限制分批，优先在段落边界（双换行）切割

    分割策略（参考 aiyxdata_tradar splitter.py 的原子性保证）：
    1. 优先按段落（双换行 \\n\\n）拆分
    2. 段落仍超限时，按单行（\\n）拆分
...
<html><head><meta charset="utf-8"><title>AIYXDATA-TRADAR 通知</title>
...
def _send_feishu(webhook_url: str, content: str, title: str) -> Dict:
    """飞书发送（纯文本消息，与 aiyxdata_tradar send_to_feishu 一致）
...
def _send_email(
    ...
    try:
        ...
        msg["From"] = formataddr(("AIYXDATA-TRADAR", from_email))
...
def _send_ntfy(server_url: str, topic: str, content: str, title: str, token: str = "") -> Dict:
    """ntfy 发送（接收已适配的 Markdown，与 aiyxdata_tradar send_to_ntfy 一致）
    ...
    headers = {
        ...
        "Markdown": "yes",
        "Title": "AIYXDATA-TRADAR Notification",  # ASCII，避免 HTTP header 编码问题
        "Priority": "default",
        "Tags": "news",
    }
    if token:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # 速率限制，等待后重试一次（与 aiyxdata_tradar 一致）
            time.sleep(10)
...
class NotificationTools:
    ...
    def _dispatch_to_channel(
        ...
        """
        # 从 config 读取批次配置（与 aiyxdata_tradar 一致）
    ...
    def _send_batched_ntfy(
        ...
        for i, (srv, topic) in enumerate(zip(servers, topics)):
            ...
            if not (srv and topic):
                continue
            # ntfy.sh 公共服务器用 2s 间隔（与 aiyxdata_tradar 一致）
