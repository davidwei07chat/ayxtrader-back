...
class SystemManagementTools:
    ...
    def trigger_crawl(self, platforms: Optional[List[str]] = None, save_to_local: bool = False, include_url: bool = False) -> Dict:
        ...
        try:
            import time
            import yaml
            from aiyxdata_tradar.crawler.fetcher import DataFetcher
            from aiyxdata_tradar.storage.local import LocalStorageBackend
            from aiyxdata_tradar.storage.base import convert_crawl_results_to_news_data
            from aiyxdata_tradar.utils.time import get_configured_time, format_date_folder, format_time_filename
            ...
            # 获取当前时间（统一使用 aiyxdata_tradar 的时间工具）
...
"""
    ...
    def check_version(self, proxy_url: Optional[str] = None) -> Dict:
        ...
        同时检查 AIYXDATA-TRADAR 和 MCP Server 两个组件的版本更新。
        远程版本 URL 从 config.yaml 获取：
        - version_check_url: AIYXDATA-TRADAR 版本
        ...
        Returns:
            版本检查结果字典，包含：
            - success: 是否成功
            - aiyxdata_tradar: AIYXDATA-TRADAR 版本检查结果
        ...
        try:
            # 导入本地版本
            from aiyxdata_tradar import __version__ as aiyxdata_tradar_version
            ...
            aiyxdata_tradar_url = advanced_config.get(
                "version_check_url",
                "https://raw.githubusercontent.com/AIYXDATA/AIYXDATA-TRADAR/refs/heads/master/version"
            )
            mcp_url = advanced_config.get(
                "mcp_version_check_url",
                "https://raw.githubusercontent.com/AIYXDATA/AIYXDATA-TRADAR/refs/heads/master/version_mcp"
            )
            ...
            # 检查两个版本
            aiyxdata_tradar_result = check_single_version(
                "AIYXDATA-TRADAR", aiyxdata_tradar_version, aiyxdata_tradar_url, proxies, headers
            )
            ...
            # 判断是否有任何更新
            any_update = (
                (aiyxdata_tradar_result.get("success") and aiyxdata_tradar_result.get("need_update", False)) or
            ...
            return {
                "success": True,
                "summary": {
                    "description": "版本检查结果（AIYXDATA-TRADAR + MCP Server）",
                    "any_update": any_update
                },
                "data": {
                    "aiyxdata_tradar": aiyxdata_tradar_result,
                    "mcp": mcp_result,
                    "any_update": any_update
                }
            }
