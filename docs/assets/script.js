...
// 本地默认配置文件 URL：由配置编辑器后台静态文件提供
const DEFAULT_CONFIG_URL = './defaults/config.yaml';
const DEFAULT_FREQUENCY_URL = './defaults/frequency_words.txt';
const DEFAULT_TIMELINE_URL = './defaults/timeline.yaml';
const DEFAULT_ANALYSIS_PROMPT_URL = './defaults/ai_analysis_prompt.txt';
const DEFAULT_TRANSLATION_PROMPT_URL = './defaults/ai_translation_prompt.txt';
const DEFAULT_VERSION_URL = './defaults/version_configs';
...
const CORE_CONFIG_FILES = [
    {
        ...
        label: '主配置',
        description: '系统配置、平台、AI、通知等',
        icon: 'fa-file-code',
        color: 'text-blue-400',
        inputId: 'load-config-yaml',
        url: DEFAULT_CONFIG_URL,
        editorId: 'yaml-editor',
        backdropId: 'yaml-backdrop',
        validatesYaml: true
    },
    {
        type: 'frequency',
        ...
        label: '频率词',
        description: '关键词组、过滤规则、正则逻辑',
        icon: 'fa-filter',
        color: 'text-orange-400',
        inputId: 'load-frequency-txt',
        url: DEFAULT_FREQUENCY_URL,
        editorId: 'frequency-editor',
        backdropId: 'frequency-backdrop'
    },
    {
        type: 'timeline',
        fileName: 'timeline.yaml',
        label: '时间线',
        description: '调度时间线、预设模板、自定义时间段',
        icon: 'fa-calendar-week',
        color: 'text-purple-400',
        inputId: 'load-timeline-yaml',
        url: DEFAULT_TIMELINE_URL,
        editorId: 'timeline-editor',
        backdropId: 'timeline-backdrop',
        validatesYaml: true
    },
    {
        type: 'analysis_prompt',
        fileName: 'ai_analysis_prompt.txt',
        label: '分析提示词',
        description: 'AI 深度分析研判使用的系统提示词',
        icon: 'fa-brain',
        color: 'text-pink-400',
        inputId: 'load-analysis-prompt-txt',
        url: DEFAULT_ANALYSIS_PROMPT_URL,
        editorId: 'analysis_prompt-editor',
        backdropId: 'analysis_prompt-backdrop'
    },
    {
        type: 'translation_prompt',
        fileName: 'ai_translation_prompt.txt',
        label: '翻译提示词',
        description: 'AI 翻译功能使用的系统提示词',
        icon: 'fa-language',
        color: 'text-emerald-400',
        inputId: 'load-translation-prompt-txt',
        url: DEFAULT_TRANSLATION_PROMPT_URL,
        editorId: 'translation_prompt-editor',
        backdropId: 'translation_prompt-backdrop'
    }
];

const CORE_CONFIG_BY_TYPE = Object.fromEntries(CORE_CONFIG_FILES.map(file => [file.type, file]));
const FREQUENCY_GROUP_TAGS = ['热榜', '地区', '财经', '科技', '文旅', '教育', '军事', '国际', '其他'];
const SOURCE_MANUAL_TAGS = FREQUENCY_GROUP_TAGS;
const SOURCE_FIXED_TAGS = {
    platforms: '热榜',
    rss: 'RSS源'
};
...
function getSourceManualTagOptions(sourceType) {
    const fixedTag = SOURCE_FIXED_TAGS[sourceType];
    return SOURCE_MANUAL_TAGS.filter(tag => tag !== fixedTag);
}

function normalizeSourceTags(tags, sourceType) {
    const rawTags = Array.isArray(tags)
        ? tags
        : typeof tags === 'string'
            ? tags.split(',')
            : [];
    const allowed = new Set(getSourceManualTagOptions(sourceType));
    const result = [];
    rawTags.forEach(tag => {
        const normalized = String(tag || '').trim();
        if (normalized && allowed.has(normalized) && !result.includes(normalized)) {
...
function yamlTagArray(tags, sourceType) {
    return `[${normalizeSourceTags(tags, sourceType).map(yamlQuote).join(', ')}]`;
}

function renderSourceTagBadges(sourceType, source) {
    const fixedTag = SOURCE_FIXED_TAGS[sourceType];
    const manualTags = normalizeSourceTags(source && source.tags, sourceType);
    const fixedChip = `<span class="source-tag-chip source-tag-fixed">${escapeHtml(fixedTag)}</span>`;
    const manualChips = manualTags.map(tag => `<span class="source-tag-chip">${escapeHtml(tag)}</span>`).join('');
    const requiredChip = manualTags.length
        ? ''
        : `<span class="source-tag-required"><i class="fa-solid fa-circle-exclamation mr-1"></i>必须选择标签</span>`;
    return `<div class="source-tag-row">${fixedChip}${manualChips}${requiredChip}</div>`;
}

function renderSourceTagSelector(sourceType, source, index) {
    const selected = normalizeSourceTags(source && source.tags, sourceType);
    const options = getSourceManualTagOptions(sourceType);
    return `
        <details class="source-tag-dropdown" onclick="event.stopPropagation()">
            <summary title="选择分类标签">
                <i class="fa-solid fa-tags"></i>
                <span>标签</span>
            </summary>
            <div class="source-tag-menu">
                <div class="source-tag-menu-title">分类标签（必选，可多选）</div>
                ${options.map(tag => `
                    <label class="source-tag-option">
                        <input type="checkbox"
                               ${selected.includes(tag) ? 'checked' : ''}
                               onchange="updateSourceTags('${sourceType}', ${index}, '${tag}', this.checked)">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 页面打开时以服务器运行配置为准，避免旧浏览器草稿遮住新部署的数据源。
    // LocalStorage 只作为读取失败前的临时兜底，不再阻止自动读取服务器配置。
    reloadAllConfigsFromServer(true);
});

window.addEventListener('resize', () => {
    window.clearTimeout(window._fileTabFitTimer);
    window._fileTabFitTimer = window.setTimeout(fitFileTabs, 80);
});
...
function saveConfigToLocalStorage() {
    try {
        if (currentYaml && currentYaml.trim().length > 10) {
            const validation = validateSourceRequiredTags(currentYaml, { showToast: false, focus: false });
            if (!validation.valid) {
                return;
            }
...
function saveFrequencyToLocalStorage() {
    try {
        const frequencyEditor = document.getElementById('frequency-editor');
        const content = frequencyEditor ? frequencyEditor.value : currentFrequency;
        if (content && content.trim().length > 10) {
            const validation = validateFrequencyGroupTags(content, { showToast: false, focus: false });
            if (!validation.valid) {
                return;
            }
            const now = new Date().toISOString();
...
window.openLoadConfigModal = function() {
    ...
    const fileOptions = CORE_CONFIG_FILES.map(file => `
                <label class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 hover:bg-blue-50 hover:border-blue-300 cursor-pointer transition-colors">
                    <input type="checkbox" id="${file.inputId}" checked class="w-4 h-4 text-blue-600 rounded">
                    <div class="flex-1">
                        <div class="font-medium text-gray-800">${file.fileName}</div>
                        <div class="text-xs text-gray-500">${file.description}</div>
                    </div>
                    <i class="fa-solid ${file.icon} ${file.color}"></i>
                </label>
    `).join('');
    modal.innerHTML = `
        <div class="modal-content" style="max-width: 520px;">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-bold text-gray-800"><i class="fa-solid fa-cloud-arrow-down mr-2 text-blue-500"></i>加载默认设置</h3>
            ...
            <div class="text-sm text-gray-600 mb-4">
                以下 5 个核心配置文件为系统固定文件，不可移动、不可删除。可选择要恢复默认内容的文件：
            </div>
            <div class="space-y-3">
                ${fileOptions}
            </div>
            <div class="text-xs text-gray-400 mt-3 p-2 bg-gray-50 rounded">
                ...
                数据来源：AiYX DATA RADAR 内置默认配置。部署时这些默认文件会随 Docker 镜像一起提供。
            </div>
...
function applyDefaultContent(type, text) {
    const file = CORE_CONFIG_BY_TYPE[type];
    if (!file) {
        throw new Error(`未知配置文件类型: ${type}`);
    }

    if (file.validatesYaml) {
        try {
            jsyaml.load(text);
        } catch (yamlErr) {
            throw new Error(`${file.fileName} YAML 语法错误: ${yamlErr.message}`);
        }
    }

    const editor = document.getElementById(file.editorId);
    if (!editor) {
        throw new Error(`${file.fileName} 编辑器不存在`);
    }
    ...
    if (type === 'config') {
        currentYaml = text;
        updateBackdrop(file.editorId, file.backdropId);
        syncYamlToUI();
        ...
        updateBackdrop(file.editorId, file.backdropId);
        syncTimelineToUI();
    } else if (type === 'analysis_prompt') {
        currentAnalysisPrompt = text;
        updateBackdrop(file.editorId, file.backdropId);
    } else if (type === 'translation_prompt') {
        currentTranslationPrompt = text;
        updateBackdrop(file.editorId, file.backdropId);
    }
}

window.confirmLoadConfig = async function() {
    const selectedFiles = CORE_CONFIG_FILES.filter(file => document.getElementById(file.inputId)?.checked);
    ...
    showToast('正在加载默认配置...', 'info');

    try {
        const promises = selectedFiles.map(file => fetch(file.url).then(res => ({ file, res })));

        const results = await Promise.all(promises);

        for (const { file, res } of results) {
            if (!res.ok) {
                throw new Error(`${file.fileName} 加载失败: ${res.status}`);
            }

            const text = await res.text();
            applyDefaultContent(file.type, text);
        }


        saveToLocalStorage();

        const loadedFiles = selectedFiles.map(file => file.fileName);
    ...
    switch(mod.key) {
        case "platforms":
            ...
            html += `<div class="flex items-center gap-2 mt-3">
                        ...
                        <button type="button" class="text-xs bg-gray-100 text-gray-600 px-3 py-1.5 rounded border border-gray-200 flex items-center gap-1 cursor-default">
...
function fitFileTabs() {
    const tabbar = document.querySelector('.file-tabbar');
    if (!tabbar) return;
    ...
    const baseLabelSize = 0.58;
    tabbar.style.setProperty('--file-tab-font-size', `${baseFontSize}rem`);
    tabbar.style.setProperty('--file-tab-label-size', `${baseLabelSize}rem`);

    const tabStyle = window.getComputedStyle(tabbar);
    const columnGap = parseFloat(tabStyle.columnGap || tabStyle.gap || '0') || 0;
    const availablePerTab = Math.max(0, (tabbar.clientWidth - columnGap * (tabs.length - 1)) / tabs.length);
    const requiredWidth = tabs.reduce((max, tab) => {
        const icon = tab.querySelector('.tab-icon');
        const file = tab.querySelector('.tab-file');
        if (!file) return max;

        const tabStyles = window.getComputedStyle(tab);
        const paddingWidth = parseFloat(tabStyles.paddingLeft) + parseFloat(tabStyles.paddingRight);
        const gapWidth = parseFloat(tabStyles.columnGap || tabStyles.gap || '0') || 0;
        const iconWidth = icon ? icon.getBoundingClientRect().width : 0;
        return Math.max(max, file.scrollWidth + iconWidth + gapWidth + paddingWidth);
    }, 0);
    const ratio = requiredWidth > 0 ? availablePerTab / requiredWidth : 1;
    const fontSizeRem = Math.max(0.46, Math.min(baseFontSize, baseFontSize * ratio));
    const labelSizeRem = Math.max(0.46, Math.min(baseLabelSize, baseLabelSize * ratio));

    tabbar.classList.toggle('is-tight', ratio < 1);
    tabbar.style.setProperty('--file-tab-font-size', `${fontSizeRem.toFixed(3)}rem`);
    tabbar.style.setProperty('--file-tab-label-size', `${labelSizeRem.toFixed(3)}rem`);
}
...
function getFrequencyGroupDisplayName(group, index) {
    if (!group) return `#${index + 1}`;
    if (group.type === 'group-name') {
        return group.name || `#${index + 1} 组别名`;
    }
    if ((group.type === 'alias' || group.type === 'alias-group') && group.items && group.items.length > 0) {
        const aliases = group.items
            .map(item => (item.alias || item.keyword || '').trim())
            .filter(Boolean);
        return aliases.length > 0 ? aliases.join(' / ') : `#${index + 1} 别名组`;
    }
    if (group.keywords && group.keywords.length > 0) {
        return group.keywords.slice(0, 2).join(' / ');
    }
    return `#${index + 1}`;
}

function getGroupsMissingRequiredTags(data) {
    return (data.wordGroups || [])
        .map((group, index) => ({ group, index }))
        .filter(({ group }) => !(group.tags || []).filter(tag => tag && tag.trim()).length);
}

function focusFrequencyGroup(index) {
    switchTab('frequency');
    setTimeout(() => {
        const card = document.querySelector(`.word-group-card[data-group-index="${index}"]`);
        if (!card) return;
        card.scrollIntoView({ behavior: 'smooth', block: 'center' });
        card.classList.add('missing-tags-focus');
        setTimeout(() => card.classList.remove('missing-tags-focus'), 1800);
    }, 80);
}

function validateFrequencyGroupTags(content, options = {}) {
    const data = parseFrequencyText(content || '');
    const missing = getGroupsMissingRequiredTags(data);
    if (!missing.length) {
        return { valid: true, data, missing };
    }

    const first = missing[0];
    const groupName = getFrequencyGroupDisplayName(first.group, first.index);
    if (options.showToast !== false) {
        showToast(`关键词组 #${first.index + 1}「${groupName}」缺少分类标签，所有关键词组至少选择一个标签后才能保存。`, 'error');
    }
    if (options.focus !== false) {
        currentFrequencyData = data;
        renderFrequencyPanel(data);
        focusFrequencyGroup(first.index);
    }
    return { valid: false, data, missing };
}

function getSourcesMissingRequiredTagsFromDoc(doc) {
    const missing = [];
    const platforms = doc?.platforms?.sources || [];
    const feeds = doc?.rss?.feeds || [];

    platforms.forEach((source, index) => {
        if (!normalizeSourceTags(source?.tags, 'platforms').length) {
            missing.push({
                sourceType: 'platforms',
                sectionKey: 'platforms',
                index,
                label: source?.name || source?.id || `平台 #${index + 1}`,
                fixedTag: SOURCE_FIXED_TAGS.platforms
            });
        }
    });

    feeds.forEach((source, index) => {
        if (!normalizeSourceTags(source?.tags, 'rss').length) {
            missing.push({
                sourceType: 'rss',
                sectionKey: 'rss',
                index,
                label: source?.name || source?.id || `RSS 源 #${index + 1}`,
                fixedTag: SOURCE_FIXED_TAGS.rss
            });
        }
    });
...
function focusSourceItem(sourceType, index) {
    switchTab('config');
    const moduleKey = sourceType === 'rss' ? 'rss' : 'platforms';
    setTimeout(() => {
        const moduleCard = document.getElementById(`module-${moduleKey}`);
        if (moduleCard) {
            moduleCard.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
        const selector = sourceType === 'rss'
            ? `.rss-feed-item[data-index="${index}"]`
            : `.platform-item[data-index="${index}"]`;
        const item = document.querySelector(selector);
        if (!item) return;
        item.scrollIntoView({ behavior: 'smooth', block: 'center' });
        item.classList.add('missing-tags-focus');
        setTimeout(() => item.classList.remove('missing-tags-focus'), 1800);
    }, 120);
}

function validateSourceRequiredTags(content, options = {}) {
    let doc = null;
    try {
        doc = jsyaml.load(content || '');
    } catch (e) {
        return { valid: true, missing: [], parseError: e };
    }

    const missing = getSourcesMissingRequiredTagsFromDoc(doc || {});
    if (!missing.length) {
    ...
    const first = missing[0];
    if (options.showToast !== false) {
        const typeName = first.sourceType === 'rss' ? 'RSS 源' : '热榜平台';
        showToast(`${typeName}「${first.label}」缺少手工分类标签；固定标签「${first.fixedTag}」会自动显示，但每个数据源仍必须至少选择一个手工标签。`, 'error');
    }
    if (options.focus !== false) {
        focusSourceItem(first.sourceType, first.index);
    }
...
function renderFrequencyPanel(data) {
    ...
    function renderGroupCard(group, idx) {
        ...
        const selectedTags = (group.tags || []).filter(tag => tag && tag.trim());
        const tagSummary = selectedTags.length > 0
            ? selectedTags.map(tag => `<span class="group-tag-chip">${tag}</span>`).join('')
            : '<span class="group-tag-required"><i class="fa-solid fa-triangle-exclamation mr-1"></i>必须选择标签</span>';
        const missingTagClass = selectedTags.length > 0 ? '' : 'missing-required-tags';
        const tagSelectorHtml = `
            <div class="mt-3 pt-3 border-t border-gray-200">
                <div class="text-xs text-gray-600 mb-2 font-bold flex items-center gap-2">
                    <span>分类标签：</span>
                    <span class="text-[10px] text-red-500 font-normal">必选，至少一个</span>
                </div>
                <div class="flex flex-wrap gap-2">
                    ${FREQUENCY_GROUP_TAGS.map(tag => {
        ...
        if (group.type === 'group-name') {
            // 组别名类型
            return `
                <div class="word-group-card border-2 border-orange-200 bg-orange-50 group ${relatedGroupStyle} ${missingTagClass} cursor-move" data-group-index="${idx}" onclick="scrollToWordGroupInEditor(${idx})">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center flex-1 gap-2">
                            ...
                            ${relatedGroupBadge}
                            <div class="group-card-tags">${tagSummary}</div>
            ...
            return `
                <div class="word-group-card border-2 border-teal-200 bg-teal-50 group ${relatedGroupStyle} ${missingTagClass} cursor-move" data-group-index="${idx}" onclick="scrollToWordGroupInEditor(${idx})">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center flex-1 gap-2">
                            ${jumpIcon}
                            ${indexBadge}
            ...
            // 连续别名组类型
            return `
                <div class="word-group-card border-2 border-purple-200 bg-purple-50 group ${relatedGroupStyle} ${missingTagClass} cursor-move" data-group-index="${idx}" onclick="scrollToWordGroupInEditor(${idx})">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center flex-1 gap-2">
                            ${jumpIcon}
                            ${indexBadge}
                    ...
                    <div class="bg-white rounded p-3 border border-purple-200 space-y-2 editable-area" onclick="event.stopPropagation()">
                        ...
                        `).join('')}
                        <button onclick="addAliasItem(${idx})"
                                class="w-full border border-dashed border-purple-300 text-purple-600 hover:bg-purple-50 hover:border-purple-500 rounded py-2 text-xs font-bold flex items-center justify-center gap-1 transition-colors">
                            <i class="fa-solid fa-plus"></i>添加别名
                        </button>
                        <div class="flex items-center justify-between mt-2">
                            <button onclick="openDeepSeekAI('group', ${idx})" class="text-xs text-blue-600 hover:text-blue-700 flex items-center gap-1">
                                <i class="fa-solid fa-wand-magic-sparkles"></i>AI 写正则
            ...
            // 普通词组类型
            return `
                <div class="word-group-card border-2 border-gray-200 bg-gray-50 group ${relatedGroupStyle} ${missingTagClass} cursor-move" data-group-index="${idx}" onclick="scrollToWordGroupInEditor(${idx})">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center flex-1 gap-2">
                            ${jumpIcon}
                            ${indexBadge}
...
window.updateAliasItem = function(groupIndex, itemIndex, field, value) {
    ...
    if (group.type === 'alias' || group.type === 'alias-group') {
        if (group.items[itemIndex]) {
            ...
            syncAliasGroupEdit(data);
        }
    }
}

function syncAliasGroupEdit(data) {
    currentFrequency = buildFrequencyText(data);
    currentFrequencyData = data;
    document.getElementById('frequency-editor').value = currentFrequency;
    updateBackdrop('frequency-editor', 'frequency-backdrop');
...
function renderPlatformsList() {
    ...
    container.innerHTML = platforms.map((p, idx) => `
        <div class="platform-item source-item ${normalizeSourceTags(p.tags, 'platforms').length ? '' : 'missing-required-tags'} bg-gray-50 rounded-lg px-3 py-2 border border-gray-200 hover:border-blue-300 transition-colors" data-index="${idx}">
            <div class="flex items-start justify-between gap-3">
                <div class="flex items-start gap-2 min-w-0">
                    <i class="fa-solid fa-grip-vertical text-gray-300 cursor-move mt-0.5"></i>
                    <div class="min-w-0">
                        <div class="flex items-center gap-2 min-w-0">
                            <span class="text-xs font-medium text-gray-700 truncate">${escapeHtml(p.name)}</span>
                            <span class="text-[10px] text-gray-400">(${escapeHtml(p.id)})</span>
                        </div>
                        ${renderSourceTagBadges('platforms', p)}
                    </div>
                </div>
                <div class="flex items-center gap-1 shrink-0">
                    ${renderSourceTagSelector('platforms', p, idx)}
                    <button onclick="removePlatform(${idx})" class="text-red-400 hover:text-red-600 text-xs px-1" title="删除">
                        <i class="fa-solid fa-trash"></i>
                    </button>
                </div>
            </div>
...
window.updateSourceTags = function(sourceType, index, tag, isChecked) {
    if (sourceType === 'platforms') {
        const platforms = parsePlatformsFromYaml();
        if (index < 0 || index >= platforms.length) return;
        platforms[index].tags = normalizeSourceTags(platforms[index].tags, 'platforms');
        if (isChecked) {
            if (!platforms[index].tags.includes(tag)) {
                platforms[index].tags.push(tag);
            }
        } else {
            platforms[index].tags = platforms[index].tags.filter(item => item !== tag);
        }
    ...
    if (sourceType === 'rss') {
        ...
        feeds[index].tags = normalizeSourceTags(feeds[index].tags, 'rss');
        if (isChecked) {
            if (!feeds[index].tags.includes(tag)) {
                feeds[index].tags.push(tag);
            }
        } else {
            feeds[index].tags = feeds[index].tags.filter(item => item !== tag);
        }
...
function updatePlatformsInYaml(platforms) {
    ...
    const newSourcesLines = platforms.map(p => {
        const tags = normalizeSourceTags(p.tags, 'platforms');
        let platformYaml = `${indent}- id: ${yamlQuote(p.id)}\n${indent}  name: ${yamlQuote(p.name)}`;
        platformYaml += `\n${indent}  tags: ${yamlTagArray(tags, 'platforms')}`;
        if (p.enabled === false) {
...
function renderRssFeedsList() {
    ...
    container.innerHTML = feeds.map((f, idx) => `
        <div class="rss-feed-item source-item ${normalizeSourceTags(f.tags, 'rss').length ? '' : 'missing-required-tags'} bg-gray-50 rounded-lg px-3 py-2 border border-gray-200 hover:border-blue-300 transition-colors" data-index="${idx}">
            <div class="flex items-start justify-between gap-3">
                <div class="flex items-start gap-2 flex-1 min-w-0">
                    <i class="fa-solid fa-rss text-orange-400 mt-0.5"></i>
                    <div class="min-w-0">
                        <div class="flex items-center gap-2 min-w-0">
                            <span class="text-xs font-medium text-gray-700 truncate">${escapeHtml(f.name)}</span>
                            <span class="text-[10px] text-gray-400">(${escapeHtml(f.id)})</span>
                            ${f.enabled === false ? '<span class="text-[9px] bg-gray-200 text-gray-500 px-1 rounded">已禁用</span>' : ''}
                        </div>
                        ${renderSourceTagBadges('rss', f)}
                    </div>
                </div>
                <div class="flex items-center gap-1 shrink-0">
                    ${renderSourceTagSelector('rss', f, idx)}
            ...
            <div class="text-[10px] text-gray-400 mt-1 truncate" title="${escapeHtml(f.url)}">${escapeHtml(f.url)}</div>
        </div>
    `).join('');
    applyLockState();
}
...
function updateRssFeedsInYaml(feeds) {
    ...
    const newFeedsLines = feeds.map(f => {
        const tags = normalizeSourceTags(f.tags, 'rss');
        let feedYaml = `${indent}- id: ${yamlQuote(f.id)}\n${indent}  name: ${yamlQuote(f.name)}\n${indent}  url: ${yamlQuote(f.url)}`;
        feedYaml += `\n${indent}  tags: ${yamlTagArray(tags, 'rss')}`;
        if (f.enabled === false) {
            feedYaml += `\n${indent}  enabled: false`;
        }
...
function openRssModalWithData(feed, editIndex) {
    ...
    modal.dataset.sourceTags = JSON.stringify(normalizeSourceTags(feed && feed.tags, 'rss'));
...
window.confirmAddRss = function() {
    ...
    newFeed.tags = normalizeSourceTags(JSON.parse(modal.dataset.sourceTags || '[]'), 'rss');
    if (maxAge) {
...
window.checkVersion = async function() {
    ...
    try {
        const versionRes = await fetch(DEFAULT_VERSION_URL);
        if (!versionRes.ok) {
        ...
        const currentFile = getCoreFileForTab(getCurrentTab());
        const fileName = currentFile.fileName;
        const currentVersion = extractVersion(getContentForCoreFile(currentFile.type));

        const latestVersion = versionMap[fileName];

        if (!latestVersion) {
             throw new Error(`未在默认版本清单中找到 ${fileName}`);
        }
...
function getCoreFileForTab(tab = currentTab) {
    return CORE_CONFIG_BY_TYPE[tab] || CORE_CONFIG_BY_TYPE.config;
}

function getContentForCoreFile(type) {
    if (type === 'config') return currentYaml;
    if (type === 'frequency') return currentFrequency;
    if (type === 'timeline') return currentTimeline;
    if (type === 'analysis_prompt') return currentAnalysisPrompt;
    if (type === 'translation_prompt') return currentTranslationPrompt;
    return '';
}
        ...
        <div class="modal-content" style="max-width: 480px;">
            ...
            ${comparison < 0 || !currentVersion ? `
                <div class="text-xs text-gray-500 bg-yellow-50 border border-yellow-200 rounded p-3 mb-4">
                    ...
                    <strong>提示：</strong>更新将加载内置默认 ${fileName}，你当前的修改将被覆盖。建议先复制保存你的自定义配置。
                </div>
            ` : ''}
...
window.updateToLatest = async function() {
    ...
    if (!confirm(`确定要用内置默认配置更新 ${fileName} 吗？\n\n你当前的自定义配置将被覆盖，建议先复制保存。`)) {
        return;
    }

    showToast('正在加载内置默认版本...', 'info');
    ...
    try {
        ...
        const text = await res.text();
        applyDefaultContent(currentFile.type, text);
    ...
    if (!data) {
        panel.innerHTML = `
            <div class="text-center py-12 text-gray-400">
                ...
                <p class="text-xs mt-1">或点击右上角「加载默认设置」</p>
            </div>`;
        return;
    }
...
function confirmSaveProfile(profileName) {
    ...
    const configContent = document.getElementById('yaml-editor')?.value || currentYaml;
    const sourceValidation = validateSourceRequiredTags(configContent);
    if (!sourceValidation.valid) {
        return;
    }
    const frequencyContent = document.getElementById('frequency-editor')?.value || currentFrequency;
    const validation = validateFrequencyGroupTags(frequencyContent);
    if (!validation.valid) {
        return;
    }
...
window.reloadAllConfigsFromServer = function(silent = false) {
    ...
    return Promise.all([
        ...
        if (hasError) {
            ...
            if (!silent) showToast('配置已从服务器读取并同步', 'success');
            // 更新保存时间
            const now = new Date().toISOString();
            localStorage.setItem(STORAGE_KEY_CONFIG_TIME, now);
            localStorage.setItem(STORAGE_KEY_FREQUENCY_TIME, now);
...
window.applyToBackend = function() {
    ...
    if (!sourceValidation.valid) {
        return Promise.resolve({
            success: false,
            error: '所有热榜平台和 RSS 源必须至少选择一个手工分类标签'
        });
    }
    const validation = validateFrequencyGroupTags(frequencyContent);
    if (!validation.valid) {
