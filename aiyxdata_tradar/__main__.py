...
class NewsAnalyzer:
    ...
    def _prepare_standalone_data(
        ...
        platform_config_map = {
            item.get("id"): item
            for item in self.ctx.platforms
            if isinstance(item, dict) and item.get("id")
        }
        rss_config_map = {
            item.get("id"): item
            for item in self.ctx.rss_feeds
            if isinstance(item, dict) and item.get("id")
        }
        ...
        for platform_id in platform_ids:
            ...
            if items:
                source_config = platform_config_map.get(platform_id, {})
            ...
            for feed_id in rss_feed_ids:
                if feed_id in feed_items_map:
                    ...
                    if max_items > 0:
                        items = items[:max_items]
                    if items:
                        source_config = rss_config_map.get(feed_id, {})
    ...
    def _reprocess_rss_data_for_effective_mode(
        self,
    ) -> Tuple[Optional[List[Dict]], Optional[List[Dict]], Optional[List[Dict]]]:
        """调度器覆盖报告模式后，按最终模式重新读取并处理已落库的 RSS 数据。"""
        if not self.ctx.rss_enabled:
            return None, None, None

        date = self.ctx.format_date()
        if self.report_mode == "current":
            rss_data = self.storage_manager.get_latest_rss_data(date)
        elif self.report_mode == "daily":
            rss_data = self.storage_manager.get_rss_data(date)
        else:
            return None, None, None

        if not rss_data:
            print(f"[RSS] 调度模式 {self.report_mode}: 未找到可重新处理的 RSS 数据")
            return None, None, None

        print(f"[RSS] 调度模式 {self.report_mode}: 重新处理 RSS 数据")
        return self._process_rss_data_by_mode(rss_data)
    ...
    def _execute_mode_strategy(
        ...
        configured_mode = self.report_mode
        ...
        if effective_mode != configured_mode:
            reprocessed_rss = self._reprocess_rss_data_for_effective_mode()
            if any(part is not None for part in reprocessed_rss):
                rss_items, rss_new_items, raw_rss_items = reprocessed_rss
